...
stringLength = hslider("h:Physical_and_Nonlinearity/v:Physical_Parameters/altFreq[2]",1,0.001,2,0.001);
...
NLFM = nonLinearModulator((nonLinearity : si.smoo),envelopeMod,altFreq,
...
fdelneck = (ma.SR/altFreq-4)*(1 - betaRatio);
vibratoEnvelope = envVibrato(vibratoBegin,vibratoAttack,100,vibratoRelease,gate);
vibrato = fdelneck + ((ma.SR/altFreq - 4)*vibratoGain*vibratoEnvelope*os.osc(vibratoFreq));
neckDelay = de.fdelay(4096,vibrato);
fdelbridge = (ma.SR/altFreq - 4)*betaRatio;
...
stereo = stereoizer(ma.SR/altFreq);
