...
#ifdef BELA_LIBPD_DISABLE_KEYBOARD
#undef BELA_LIBPD_KEYBOARD
#endif // BELA_LIBPD_DISABLE_KEYBOARD
...
static void keyboardRead(void*)
{
	...
	{
		close(fd);
		fd = open(gInputEventPath.c_str(), O_RDONLY);
		if (fd == -1) {
			if(lastWasSuccess)
				fprintf(stderr, "Cannot open %s:.\n", gInputEventPath.c_str());
			// try again later
			usleep(100000);
		...
		lastWasSuccess = true;
		printf("Opened %s for keyboard input\n", gInputEventPath.c_str());
		struct pollfd fds = {
			.fd = fd,
			.events = POLLIN,
		};
...
#endif // BELA_LIBPD_TRILL
#ifdef BELA_LIBPD_KEYBOARD
	if(0 == strcmp(source, "bela_setKeyboard"))
	{
		if(0 == strcmp(symbol, "path") && 1 == argc && libpd_is_symbol(argv))
		{
			gInputEventPath = libpd_get_symbol(argv);
			 // start qwerty kbd capture thread
			Bela_runAuxiliaryTask(keyboardRead);
		} else {
			rt_fprintf(stderr, "bela_setKeyboard. Usage: `path eventX`\n");
		}
		return;
	}
#endif // BELA_LIBPD_KEYBOARD
...
#endif // BELA_LIBPD_TRILL
#ifdef BELA_LIBPD_KEYBOARD
	gKeyboardPipe.setup("keyboardPipe", 1024);
#endif // BELA_LIBPD_KEYBOARD
	return true;
}
...
void render(BelaContext *context, void *userData)
{
	...
	{
		KeyMsg msg;
		while(gKeyboardPipe.readRt(msg) >= 0)
		{
			// msg.value: 0 is off, 1 is on, 2 is repeated
			// when receiving a repeated keystroke, the GUI first
			// sends to Pd a keyup and then a keydown
			size_t times = (2 == msg.value) ? 2 : 1;
			while(times--)
			{
				bool offOn;
				if(2 == msg.value)
